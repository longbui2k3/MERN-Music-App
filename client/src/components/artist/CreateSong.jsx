...
import { SingerAPI, SongAPI, createAlbum, searchGenres } from "../../api";
...
import { LuAlbum, LuPen } from "react-icons/lu";
...
export default function CreateSong() {
  ...
  const [inputReleasedDate, setInputReleasedDate] = useState("");
  const [inputWrittenBy, setInputWrittenBy] = useState("");
  const [inputProducedBy, setInputProducedBy] = useState("");
  ...
  const handleInputReleasedDate = async (e) => {
    setInputReleasedDate(e.target.value);
    await register("releasedDate").onChange(e);
  };
  const handleInputWrittenBy = async (e) => {
    setInputWrittenBy(e.target.value);
    await register("writtenBy").onChange(e);
  };
  const handleInputProducedBy = async (e) => {
    setInputProducedBy(e.target.value);
    await register("producedBy").onChange(e);
  };
  const handleInputAlbum = async function (e) {
    console.log(e.target.selectedIndex);
    setInputAlbum(albums[e.target.selectedIndex]);
    await register("album").onChange(e);
  };
          ...
          >
            ...
            <div className="flex justify-between">
              ...
              <div className="relative w-[30%]">
                ...
                <Input
                  ...
                  h="50px"
                  onChange={handleInputReleasedDate}
                  name={register("releasedDate").name}
                  onBlur={register("releasedDate").onBlur}
                  ref={register("releasedDate").ref}
                />
              </div>
            </div>
            <div className="flex justify-between">
              <div className="relative w-[45%]">
                ...
                <Input
                  ...
                  h="50px"
                  onChange={handleInputWrittenBy}
                  name={register("writtenBy").name}
                  onBlur={register("writtenBy").onBlur}
                  ref={register("writtenBy").ref}
                />
              </div>
              <div className="relative w-[45%]">
                <FormLabel className={"text-white mt-[10px] font-[500]"}>
                ...
                <Input
                  ...
                  h="50px"
                  onChange={handleInputProducedBy}
                  name={register("producedBy").name}
                  onBlur={register("producedBy").onBlur}
                  ref={register("producedBy").ref}
                />
              </div>
            </div>
            <div className="relative">
              ...
              <div className="relative w-full">
                <select
                  className="w-full p-2.5 text-white bg-[rgb(20,20,20)] border rounded-md shadow-sm outline-none appearance-none"
                  onChange={handleInputAlbum}
                  name={register("album").name}
                  onBlur={register("album").onBlur}
                  ref={register("album").ref}
                >
                  {albums.map((album) => (
            ...
            <div className="relative">
              <div className="flex justify-between">
                ...
                <div className="flex justify-center items-center rounded-lg relative mt-4 h-[30px] bg-[rgb(35,35,35)]">
                  ...
                  {singers ? (
                    <div className="absolute top-10 left-0 w-[230px] bg-[rgb(35,35,35)] rounded-lg z-10 overflow-hidden">
                      {singers.map((singer) => (
                        <div
                          className="py-2 px-4 cursor-pointer hover:bg-[rgb(50,50,50)]"
                          onClick={function (e) {
                            if (!inputSingers.some((s) => s._id === singer._id))
                  ...
                  {inputSingers.map((singer, i) => (
                    <div className="flex gap-2 justify-between px-2 py-1 bg-[rgb(35,35,35)] rounded-lg">
                      ...
                      <HiMiniXMark
                        ...
                        onClick={function (e) {
                          setInputSingers(
                            inputSingers.filter((singer2) => singer2 !== singer)
                          );
                        }}
                      />
                    </div>
            ...
            <div className="relative">
              <FormLabel className={"text-white mt-[10px] font-[500]"}>
                <h2 class="text-2xl font-semibold mb-2">Upload your video</h2>
                <p class="text-xs text-gray-500">
                  File should be of format .mp4, .avi, .mov or .mkv
                </p>
              </FormLabel>
              <div class="flex flex-col items-center justify-center w-full h-[200px] my-5 bg-none sm:rounded-lg sm:shadow-xl border border-dashed border-[#e0e0e0]">
                <input
                  type="file"
                  id="file-upload"
                  class="hidden"
                  accept="video/*,audio/*"
                  onChange={async function (e) {
                    const fileVideo = document.getElementById("file-upload");
                    const [file] = fileVideo.files;
                    if (file) {
                    }
                    setInputFileVideo(file);
                    await register("video").onChange(e);
                  }}
                  name={register("video").name}
                  onBlur={register("video").onBlur}
                  ref={register("video").ref}
                />
                <label
                  for="file-upload"
                  class="z-20 flex flex-col-reverse items-center justify-center w-full h-full cursor-pointer"
                >
                  <p class="z-10 text-md text-center text-white">
              ...
              {inputFileVideo ? (
                <div className="flex flex-col justify-center border border-[#e0e0e0] h-[50px] w-full py-auto px-[20px]">
                  <div className="flex justify-between">
                    <div>{inputFileVideo.name}</div>
                    <div
